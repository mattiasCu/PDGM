...
if __name__ == '__main__':
    ...
    attn = attn.reshape(-1, window_size, 3*window_size)  
    ...
    TOKEN_SELF_ATTN_VALUE = torch.tensor(TOKEN_SELF_ATTN_VALUE, dtype=torch.float)
    score = torch.where(attn==0, TOKEN_SELF_ATTN_VALUE, attn)
    sampler = GumbleSampler(top_k, 1.0, True)
   
    score = sampler(score)
    val = sampler.validate(score)
    
    accuracy = (score == val).float().mean().item()
    print(f'Accuracy: {accuracy:.4f}')
    
    score = score.reshape(num_relation, -1, window_size, 3*window_size)
